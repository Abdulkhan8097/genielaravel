...
use App\Models\ArndistributorcategoryModel;
use App\Models\DistributorCategoryMasterModel;

class Arndistributorcategorydata extends Controller
{
     ...
     * Purpose: JIRA ID: SMF-37. Helps to show list of available ARN wise distributor category data
     * Created:
     * Modified:
     * Modified by:
     */
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = ArndistributorcategoryModel::getARNDistributorCategoryDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_arn_distributor_category" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            // Pass to view
            return view('arndistributorcategorydata/list')->with($data);
        }
    }
}
