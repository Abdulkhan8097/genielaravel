...
use App\Models\PincodemasterModel;

class Pincodemaster extends Controller
{
    protected $data_table_headings;
    public function __construct(){
        $this->middleware('auth');
        $this->data_table_headings = array('pincode' => array('label' => 'Pincode'),
     ...
     * Purpose: JIRA ID: SMF-37. Helps to show list of available pincode data
     * Created:
     * Modified:
     * Modified by:
     */
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = PincodemasterModel::getMasterDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_pincode_city_state" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            // Pass to view
            return view('pincodemaster/list')->with($data);
        }
    }
}
