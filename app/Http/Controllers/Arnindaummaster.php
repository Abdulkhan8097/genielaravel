...
use App\Models\ArnindaumModel;

class Arnindaummaster extends Controller
{
    protected $data_table_headings;
    public function __construct(){
        $this->middleware('auth');
        $this->data_table_headings = array('ARN' => array('label' => 'ARN'),
                                           'total_ind_aum' => array('label' => 'Total Industry AUM'),
                                           'ind_aum_as_on_date' => array('label' => 'AUM as on date'),
     ...
     * Purpose: JIRA ID: SMF-37. Helps to show list of available ARN wise industry aum data
     * Created:
     * Modified:
     * Modified by:
     */
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = ArnindaumModel::getARNIndAumDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_arn_ind_aum_data" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            // Pass to view
            return view('arnindaumdata/list')->with($data);
        }
    }
}
