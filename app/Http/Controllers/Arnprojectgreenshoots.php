...
use App\Models\ArnprojectgreenshootsModel;

class Arnprojectgreenshoots extends Controller
{
    protected $data_table_headings;
    public function __construct(){
        $this->middleware('auth');
        $this->data_table_headings = array('ARN' => array('label' => 'ARN'),
                                           'project_green_shoots' => array('label' => 'Project Green Shoots'),
     ...
     * Purpose: JIRA ID: SMF-37. Helps to show list of available ARN wise project focus data
     * Created:
     * Modified:
     * Modified by:
     */
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = ArnprojectgreenshootsModel::getArnProjectGreenShootsDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_project_green_shoots_yes_no" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            if(!$partnersData['records']->isEmpty()){
                ...
                // coming here if no records are retrieved from MySQL table: drm_uploaded_project_green_shoots_yes_no
            ...
            // Pass to view
            return view('arnprojectgreenshoots/list')->with($data);
        }
    }
}
