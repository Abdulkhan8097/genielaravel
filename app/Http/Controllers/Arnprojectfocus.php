...
use App\Models\ArnprojectfocusModel;

class Arnprojectfocus extends Controller
{
    protected $data_table_headings;
    public function __construct(){
        $this->middleware('auth');
        $this->data_table_headings = array('ARN' => array('label' => 'ARN'),
                                           'project_focus' => array('label' => 'Project Focus'),
    ...
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = ArnprojectfocusModel::getARNProjectFocusDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_arn_project_focus_yes_no" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            // Pass to view
            return view('arnprojectfocus/list')->with($data);
        }
    }
}
