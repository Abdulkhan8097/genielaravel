...
use App\Models\ArnprojectemergingstarsModel;

class Arnprojectemergingstars extends Controller
{
    protected $data_table_headings;
    public function __construct(){
        $this->middleware('auth');
        $this->data_table_headings = array('ARN' => array('label' => 'ARN'),
                                           'project_emerging_stars' => array('label' => 'Project Emerging Stars'),
     ...
     * Purpose: JIRA ID: SMF-37. Helps to show list of available ARN wise project emerging stars data
     * Created:
     * Modified:
     * Modified by:
     */
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = ArnprojectemergingstarsModel::getARNProjectEmergingDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_arn_project_emerging_stars_yes_no" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            // Pass to view
            return view('arnprojectemergingstars/list')->with($data);
        }
    }
}
