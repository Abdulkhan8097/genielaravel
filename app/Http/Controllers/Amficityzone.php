...
use App\Models\AmficityzoneModel;

class Amficityzone extends Controller
{
    protected $data_table_headings;
    public function __construct(){
        $this->middleware('auth');
        $this->data_table_headings = array('amfi_city' => array('label' => 'AMFI City'),
                                           'mapped_zone' => array('label' => 'Zone'),
    ...
    public function index(Request $request)
    {
        ...
        if(count($request->all()) > 0){
            ...
            $partnersData = AmficityzoneModel::getAmfiCityZoneDataList($request->all());
            ...
            if($flag_export_data){
                ...
                exec('sh '. get_server_document_root(true) . '/vendor/shell_scripts/export_csv_data.sh -i"drm_uploaded_amfi_city_zone_mapping" "'. $where_conditions .'" "'. $order_by_clause .'"', $return_var, $exit_code);
            ...
            if(!$partnersData['records']->isEmpty()){
                ...
                // coming here if no records are retrieved from MySQL table: drm_uploaded_amfi_city_zone_mapping
            ...
            // Pass to view
            return view('amficityzone/list')->with($data);
        }
    }
}
