...
class ArnalternatedataModel extends Model
{
    use HasFactory;
    public static function getARNAlternateDataList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('arnalternatedata.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('arnalternatedata.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
                    ...
                    case 'status':
                        if(isset($value['search']['value']) && is_numeric($value['search']['value'])){
                            if($value['data'] == 'status'){
                                $value['data'] = 'arnalternatedata.status';
                            }
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'arnalternatedata.ARN ASC';
        }

        $records = DB::table('arn_alternate_details AS arnalternatedata')
                    ->select('arnalternatedata.ARN',
                             'arnalternatedata.alternate_mobile_1', 'arnalternatedata.alternate_mobile_2',
                             'arnalternatedata.alternate_mobile_3', 'arnalternatedata.alternate_mobile_4',
                             'arnalternatedata.alternate_mobile_5', 'arnalternatedata.alternate_email_1',
                             'arnalternatedata.alternate_email_2', 'arnalternatedata.alternate_email_3',
                             'arnalternatedata.alternate_email_4', 'arnalternatedata.alternate_email_5',
                             'arnalternatedata.status',
                             (!$flag_export_data?'arnalternatedata.created_at':DB::raw('DATE_FORMAT(arnalternatedata.created_at, "%d/%m/%Y") AS created_at')));
