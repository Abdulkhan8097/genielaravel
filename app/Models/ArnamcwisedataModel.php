...
class ArnamcwisedataModel extends Model
{
    use HasFactory;
    public static function getARNAmcDataList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('arnamcdata.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('arnamcdata.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'arnamcdata.ARN ASC';
        }

        $records = DB::table('drm_project_focus_amc_wise_details AS arnamcdata')
                    ->select('arnamcdata.ARN', 'arnamcdata.amc_name', 'arnamcdata.total_commission_expenses_paid',
                             'arnamcdata.gross_inflows', 'arnamcdata.net_inflows', 'arnamcdata.avg_aum_for_last_reported_year',
                             'arnamcdata.closing_aum_for_last_financial_year', 'arnamcdata.effective_yield',
                             'arnamcdata.nature_of_aum', 'arnamcdata.reported_year', 'arnamcdata.status',
                             (!$flag_export_data?'arnamcdata.created_at':DB::raw('DATE_FORMAT(arnamcdata.created_at, "%d/%m/%Y") AS created_at')));
