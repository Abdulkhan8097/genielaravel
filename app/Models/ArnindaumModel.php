...
class ArnindaumModel extends Model
{
    use HasFactory;
    public static function getARNIndAumDataList($input_arr = array()){
                    ...
                    case 'ind_aum_as_on_date':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('arnindaum.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('arnindaum.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'arnindaum.ARN ASC';
        }

        $records = DB::table('drm_uploaded_arn_ind_aum_data AS arnindaum')
                    ->select('arnindaum.ARN', 'arnindaum.total_ind_aum', (!$flag_export_data?'arnindaum.ind_aum_as_on_date':DB::raw('DATE_FORMAT(arnindaum.ind_aum_as_on_date, "%d/%m/%Y") AS ind_aum_as_on_date')), 'arnindaum.status',
                             (!$flag_export_data?'arnindaum.created_at':DB::raw('DATE_FORMAT(arnindaum.created_at, "%d/%m/%Y") AS created_at')));
