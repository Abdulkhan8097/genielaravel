...
class ArnprojectgreenshootsModel extends Model
{
    use HasFactory;
    public static function getArnProjectGreenShootsDataList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('arnprojectgreenshoots.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('arnprojectgreenshoots.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
                    ...
                    case 'status':
                        if(isset($value['search']['value']) && is_numeric($value['search']['value'])){
                            if($value['data'] == 'status'){
                                $value['data'] = 'arnprojectgreenshoots.status';
                            }
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'arnprojectgreenshoots.ARN ASC';
        }

        $records = DB::table('drm_uploaded_project_green_shoots_yes_no AS arnprojectgreenshoots')
                    ->select('arnprojectgreenshoots.ARN', 'arnprojectgreenshoots.project_green_shoots', 'arnprojectgreenshoots.status',
                             (!$flag_export_data?'arnprojectgreenshoots.created_at':DB::raw('DATE_FORMAT(arnprojectgreenshoots.created_at, "%d/%m/%Y") AS created_at')));
