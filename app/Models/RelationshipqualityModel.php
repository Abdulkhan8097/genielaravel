...
class RelationshipqualityModel extends Model
{
    use HasFactory;
    public static function getList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('relationship.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('relationship.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'relationship.label ASC';
        }

        $records = DB::table('drm_relationship_quality_master AS relationship')
                    ->select('relationship.label', 'relationship.description', 'relationship.score', 'relationship.status',
                             (!$flag_export_data?'relationship.created_at':DB::raw('DATE_FORMAT(relationship.created_at, "%d/%m/%Y") AS created_at')));
