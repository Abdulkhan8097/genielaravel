...
class ArnbdmmappingModel extends Model
{
    use HasFactory;
    public static function getARNBDMMappingList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('bdmmapping.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('bdmmapping.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'bdmmapping.ARN ASC';
        }

        $records = DB::table('drm_uploaded_arn_bdm_mapping AS bdmmapping')
                    ->leftJoin('users', 'bdmmapping.bdm_user_id', '=', 'users.id')
                    ->select('bdmmapping.ARN', 'bdmmapping.bdm_email', 'users.name AS bdm_name',
                             'bdmmapping.rm_relationship', 'bdmmapping.status',
                             (!$flag_export_data?'bdmmapping.created_at':DB::raw('DATE_FORMAT(bdmmapping.created_at, "%d/%m/%Y") AS created_at')));
