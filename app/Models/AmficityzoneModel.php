...
class AmficityzoneModel extends Model
{
    use HasFactory;
    public static function getAmfiCityZoneDataList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('amficityzones.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('amficityzones.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
                    ...
                    case 'status':
                        if(isset($value['search']['value']) && is_numeric($value['search']['value'])){
                            if($value['data'] == 'status'){
                                $value['data'] = 'amficityzones.status';
                            }
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'amficityzones.amfi_city ASC';
        }

        $records = DB::table('drm_uploaded_amfi_city_zone_mapping AS amficityzones')
                    ->select('amficityzones.amfi_city', 'amficityzones.mapped_zone', 'amficityzones.status',
                             (!$flag_export_data?'amficityzones.created_at':DB::raw('DATE_FORMAT(amficityzones.created_at, "%d/%m/%Y") AS created_at')));
