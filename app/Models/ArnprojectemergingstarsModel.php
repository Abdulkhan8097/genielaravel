...
class ArnprojectemergingstarsModel extends Model
{
    use HasFactory;
    public static function getARNProjectEmergingDataList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('arnprojectemerging.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('arnprojectemerging.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
                    ...
                    case 'status':
                        if(isset($value['search']['value']) && is_numeric($value['search']['value'])){
                            if($value['data'] == 'status'){
                                $value['data'] = 'arnprojectemerging.status';
                            }
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'arnprojectemerging.ARN ASC';
        }

        $records = DB::table('drm_uploaded_arn_project_emerging_stars_yes_no AS arnprojectemerging')
                    ->select('arnprojectemerging.ARN', 'arnprojectemerging.project_emerging_stars', 'arnprojectemerging.status',
                             (!$flag_export_data?'arnprojectemerging.created_at':DB::raw('DATE_FORMAT(arnprojectemerging.created_at, "%d/%m/%Y") AS created_at')));
