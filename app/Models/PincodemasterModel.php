...
class PincodemasterModel extends Model
{
    use HasFactory;
    public static function getMasterDataList($input_arr = array()){
                    ...
                    case 'created_at':
                        if(isset($value['search']['value']) && !empty($value['search']['value']) && strpos($value['search']['value'], ';') !== FALSE){
                            ...
                            if(isset($searching_dates[0]) && !empty($searching_dates) && strtotime($searching_dates[0]) !== FALSE && isset($searching_dates[1]) && !empty($searching_dates) && strtotime($searching_dates[1]) !== FALSE){
                                ...
                                $where_conditions[] = array('pincode.'. $value['data'], '>=', $searching_dates[0] .' 00:00:00');
                                $where_conditions[] = array('pincode.'. $value['data'], '<=', $searching_dates[1] .' 23:59:59');
                            }
                            else{
        ...
        if(isset($order[0]['column']) && isset($order[0]['dir']) && isset($columns[$order[0]['column']]['data']) && !empty($columns[$order[0]['column']]['data'])){
            ...
            $order_by_clause = 'pincode.pincode ASC';
        }

        $records = DB::table('drm_uploaded_pincode_city_state AS pincode')
                    ->select('pincode.pincode', 'pincode.city', 'pincode.state', 'pincode.status',
                             (!$flag_export_data?'pincode.created_at':DB::raw('DATE_FORMAT(pincode.created_at, "%d/%m/%Y") AS created_at')));
