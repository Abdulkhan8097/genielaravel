@extends('../layout')
@section('title', 'MIS')
@section('breadcrumb_heading', 'MIS')
...
@section('content')
			...
			$order_type_row = [];
			foreach ($mis_order_types_arr as $order_type_key => $order_type_label) {
			switch ($order_type_key) {
				case 'initiated_smart_switch':
				case 'uploaded_cas_successfully':
				case 'checked_improvements':
				case 'review_and_otp_pending':
				case 'confirm_switch':
					break;
				default:
					$order_type_row = array_merge($order_type_row, [$order_type_key => ['label' => $order_type_label]]);
			}
			}
			unset($order_type_key, $order_type_label);

			$data_table_headings = array_merge($data_table_headings, $order_type_row, ['created' => ['label'=>'Date Created'], 'modified' => ['label'=>'Date Modified']]);
...
<script type="text/javascript">
      ...
      case 'modified':
        ...
        break;
      case 'order_status':
        var record_status_arr = JSON.parse('{!! $uploaded_cas_order_status_json !!}');
        var selected = '';
        $.each(record_status_arr, function(key, value){
            selected = '';
            if(value.label.toLowerCase() == 'all'){
                selected = 'selected';
            }
            txtSearchInput += '<option value="'+ value.value +'" '+ selected +'>'+ value.label +'</option>';
        });

        // browser-default class used below to avoid by default conversion of HTML dropdowns to material_select dropdowns. Reference: https://materializecss.com/select.html
        // method used to convert dropdowns to material_select: $('select').material_select(); it's present in "js/admin/plugins.js"
        // to remove existing material_select initialized dropdowns you can use: $('select').material_select(); then after just use $('select').show(); to make them visible
        txtSearchInput = '<select class="form-control">'+ txtSearchInput +'</select>';
        break;
      case 'number_of_switch_out_orders':
      case 'number_of_redemption_on_orders_placed':
      case 'number_of_redemption_on_orders_rejected_by_bse':
      case 'number_of_redemption_on_orders_accepted_by_bse':
      case 'number_of_redemption_on_orders_accepted_by_rta':
      case 'number_of_redemption_on_orders_rejected_by_rta':
      case 'number_of_redemption_on_orders_pending_status_by_rta':
      case 'number_of_purchase_orders_to_be_placed':
      case 'number_of_purchase_orders_placed':
      case 'number_of_purchase_orders_for_which_units_allowed':
        txtSearchInput = '&nbsp;';
        $.extend(columnDefJSON, {"orderable":false});
        break;
      case 'action':
      case 'total_number_of_schemes_detected_in_cas':
      case 'total_number_of_schemes_suggested_for_smartswitch':
      case 'actual_number_of_schemes_selected_by_client':
      case 'total_value_of_schemes_suggested_for_smartswitch':
      case 'total_value_of_schemes_selected_by_client_for_smartswitch':
      case 'percentage_share_of_schemes_proceeded_by_count_c_by_b':
      case 'percentage_share_of_schemes_proceeded_by_count_e_by_d':
        txtSearchInput = '&nbsp;';
        $.extend(columnDefJSON, {"orderable":false, "visible":false});
        break;
      default:
	...
	function filter_mis_data(inputObj) 
	{
        ...
        // if (flagExportData) {
        //     var exportFormObj = $('#frm_export_data');
        //     $.each(inputData, function (index, value) {
        //         exportFormObj.append('<input type="hidden" name="' + value.name + '" value="' + value.value + '">');
        //     });
			
        //     exportFormObj.append('<input type="hidden" name="export_data" value="1">');
        //     exportFormObj.append('<input type="hidden" name="partner_interswitch_mis_request" value="1">');
        //     exportFormObj.append('<input type="hidden" name="records_created_by_bdm" value="{{ $records_created_by_bdm }}">');
        //     exportFormObj.append('<input type="hidden" name="searched_order_source" value="' + searchedOrderSource + '">');

        //     // exportFormObj.attr({'action': "{{ url('get-mis-data') }}"});
		// 	exportFormObj.attr({'action': baseurl + '/get-mis-data'});
        //     exportFormObj.submit();
        //     exportFormObj.attr({'action': 'javascript:void(0);'});
        //     exportFormObj.empty();
        // } 
		if (flagExportData) {
