...
$project_focus_json = array(array('key' => '', 'value' => 'All'),
                          array('key' => 'no', 'value' => 'no'),
                          array('key' => 'yes', 'value' => 'yes'));
$project_focus_json = json_encode($project_focus_json);
// preparing JSON values which are getting used in JAVASCRIPT for creating dropdown options. ENDS
@endphp
@extends('../layout')
@section('title', 'ARN Wise Project Focus Data List')
@section('breadcrumb_heading', 'ARN Wise Project Focus Data List')
...
@section('custom_scripts')
    ...
    function export_csv_formatted_data(inputObj){
        ...
        formObj.attr({'action': baseurl + '/arnprojectfocus'});
        formObj.submit();
        ...
        $('#panel_table_sm thead tr:nth-child(1) th').each(function(idx){
            ...
            switch(data_column){
                ...
                case 'project_focus':
                    ...
                    else if(data_column == 'project_focus'){
                        dropdown_filter_options = JSON.parse(@json($project_focus_json));
                    }
                    else{
