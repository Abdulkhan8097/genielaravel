...
$project_green_shoots_json = array(array('key' => '', 'value' => 'All'),
                          array('key' => 'no', 'value' => 'no'),
                          array('key' => 'yes', 'value' => 'yes'));
$project_green_shoots_json = json_encode($project_green_shoots_json);
// preparing JSON values which are getting used in JAVASCRIPT for creating dropdown options. ENDS
@endphp
@extends('../layout')
@section('title', 'ARN Wise Project Green Shoots List')
@section('breadcrumb_heading', 'ARN Wise Project Green Shoots List')
...
@section('custom_scripts')
    ...
    function export_csv_formatted_data(inputObj){
        ...
        formObj.attr({'action': baseurl + '/arnprojectgreenshoots'});
        formObj.submit();
    ...
    $(document).ready(function() {
        ...
        $('#panel_table_sm thead tr:nth-child(1) th').each(function(idx){
            ...
            switch(data_column){
                ...
                case 'project_green_shoots':
                    ...
                    else if(data_column == 'project_green_shoots'){
                        dropdown_filter_options = JSON.parse(@json($project_green_shoots_json));
                    }
                    else{
        ...
        data_table = $('#panel_table_sm').DataTable({
            ...
            "scrollX": true,
            "ajax": {
                "url": baseurl + "/arnprojectgreenshoots",
                "type": "POST",
                "data": function(d){
                    d.load_datatable = 1;
                },
