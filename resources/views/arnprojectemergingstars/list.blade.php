...
$project_emerging_stars_json = array(array('key' => '', 'value' => 'All'),
                          array('key' => 'no', 'value' => 'no'),
                          array('key' => 'yes', 'value' => 'yes'));
$project_emerging_stars_json = json_encode($project_emerging_stars_json);
// preparing JSON values which are getting used in JAVASCRIPT for creating dropdown options. ENDS
@endphp
@extends('../layout')
@section('title', 'ARN Wise Project Emerging Stars Data List')
@section('breadcrumb_heading', 'ARN Wise Project Emerging Stars Data List')
...
@section('custom_scripts')
    ...
    function export_csv_formatted_data(inputObj){
        ...
        formObj.attr({'action': baseurl + '/arnprojectemergingdata'});
        formObj.submit();
    ...
    $(document).ready(function() {
        ...
        $('#panel_table_sm thead tr:nth-child(1) th').each(function(idx){
            ...
            switch(data_column){
                ...
                case 'project_emerging_stars':
                    ...
                    else if(data_column == 'project_emerging_stars'){
                        dropdown_filter_options = JSON.parse(@json($project_emerging_stars_json));
                    }
                    else{
                ...
                case 'action':
                    txtSearchInput = '&nbsp;';
                    if($.inArray(data_column, ['total_commission_expenses_paid', 'gross_inflows', 'net_inflows', 'avg_aum_for_last_reported_year', 'closing_aum_for_last_financial_year', 'effective_yield']) == -1){
                        $.extend(columnDefJSON, {"orderable":false});
                    }
                    break;
                default:
        ...
        data_table = $('#panel_table_sm').DataTable({
            ...
            "scrollX": true,
            "ajax": {
                "url": baseurl + "/arnprojectemergingdata",
                "type": "POST",
                "data": function(d){
                    d.load_datatable = 1;
                },
