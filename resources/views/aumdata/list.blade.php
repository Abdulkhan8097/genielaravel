...
$aum_year_arn_record_json[] =["key" => "", "value" => "Selec Year"];
foreach($getListofYears as $year){
    $aum_year_arn['key'] = $year;
    $aum_year_arn['value'] = $year;
    $aum_year_arn_record_json[] = $aum_year_arn;
    
}

$aum_year_arn_record_json = json_encode($aum_year_arn_record_json);

@endphp
@extends('../layout')
@section('title', 'AUM Data List')
@section('breadcrumb_heading', 'AUM Data List')
...
@section('custom_scripts')
    ...
    function export_csv_formatted_data(inputObj){
        ...
        formObj.attr({'action': baseurl + '/aumdatalist'});
        formObj.submit();
        ...
        $('#panel_table_sm thead tr:nth-child(1) th').each(function(idx){
            ...
            switch(data_column){
                ...
                case 'aum_year':
                    ...
                    if(data_column == 'aum_year'){
                        dropdown_filter_options = JSON.parse(@json($aum_year_arn_record_json));
                    }
                    else{
                        dropdown_filter_options = [{"key":"2023", "value":"2023"},
                                                   {"key":"2022", "value":"2022"},
                                                   {"key":"2021", "value":"2021"},
                                                   {"key":"2020", "value":"2020"}
                                                ];
                ...
                case 'arn_yield':
                    txtSearchInput = '&nbsp;';
                    if($.inArray(data_column, ['arn_avg_aum', 'arn_total_commission', 'arn_yield']) == -1){
                        $.extend(columnDefJSON, {"orderable":false});
                    }
                    break;
                default:
