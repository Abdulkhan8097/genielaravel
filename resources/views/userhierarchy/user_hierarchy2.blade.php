...
<link rel='stylesheet prefetch' href='https://fonts.googleapis.com/css?family=Roboto'>
 ...
 <link rel="stylesheet" type="text/css" href="{{asset('css/user-Hierarchy.css?t=1695212821')}}">
...
<div class="row mt-4">
    <div class="col-lg-12">
		<div class="mt-2">
			<div class="row">
				<div class="col-lg-12">
					<div id="full-container" style="margin-bottom: 50px;">
			            ...
			            <button class=" btn-action btn-zoomIn"> Zoom In <span class='icon'/> <i class="fa fa-plus" aria-hidden="true"></i></span></button>

			            <button class=" btn-action btn-zoomOut"> Zoom Out <span class='icon'/> <i class="fa fa-minus" aria-hidden="true"></i></span></button>

			            <?php /*<button class=" btn-action btn-back" onclick="params.funcs.back()"> Back <span class='icon'/> <i class="fa fa-arrow-left" aria-hidden="true"></i></span></button>
      		...
      		<div class="user-search-box">
        			<div class="input-box">
      				...
      				<div class="input-wrapper">
        					...
        					<div class="input-bottom-placeholder">By Name</div>
      				</div>
          			<div></div>
        			</div>
...
function drawOrganizationChart(params) {
  ...
  var center = [params.chartWidth /2, params.chartHeight /2];
  ...
  var diagonal = d3.svg.diagonal()
    .projection(function(d) {
      /*if(d.status == 'Inactive' && (d._children !== undefined || d.children !== undefined)){
        console.log('ddd Inactive ===> ', d)
  ...
  //necessary so that zoom knows where to zoom and unzoom from
    ...
    function zoomed() {
      svg.attr("transform", "translate(" + zoomBehaviours.translate() + ") scale(" + zoomBehaviours.scale() + ")");
      //svg.attr("transform","translate(" + d3.event.translate + ")" +" scale(" + d3.event.scale + ")");
    }


    d3.select('.btn-zoomIn').on('click', function(){
      d3.event.preventDefault();

      var scale = zoomBehaviours.scale(),
        extent = zoomBehaviours.scaleExtent(),
        translate = zoomBehaviours.translate(),
        ...
        d3.transition().duration(350).tween("zoom", function () {
            ...
            return function (t) {
                zoomBehaviours.scale(interpolate_scale(t))
    ...
    d3.select('.btn-zoomOut').on('click', function(){
      d3.event.preventDefault();
  ...
  function update(source, param) {
    ...
    .attr("data-node-group-id",function(d){
      ...
      .attr("class", function(d) {
        ...
        res += ((d._children || d.children ) && d.status == 'Inactive' ? " nodeHasChildrenInActive" : " nodeHasChildren" );
        return res;
      });
    ...
    nodeGroup.append("text")
      ...
      .text(function(d) {
          ...
          if(d.children || d._children){
      		...
      		//console.log('children ===> ', d.parent.children);
      ...
      .text(function(d) {
          ...
          if(d.children || d._children){
            $(this).addClass('white-color');
      		...
      		//$(this).attr('class', 'emp-count-icon white-color');
      	}
    ...
    function tooltipContent(item) {
      ...
      strVar += "      <a class=\"name\" > " + item.name + "<\/a>";
      ...
      parentLoop:
        for (var i = 0; i < secondLevelChildren.length; i++) {
          ...
          for (var j = 0; j < thirdLevelChildren.length; j++) {
            ...
            if (thirdLevelChild.unit.value  == item.value ) {
  ...
  function reflectResults(results) {
    var htmlStringArray = results.map(function(result) {
      ...
      strVar += "              <p class=\"name\">" + result.name + ' - ' + result.status + "<\/p>";
  ...
  function findInTree(rootElement, searchText) {
    ...
    function recursivelyFindIn(user) {
      if (user.name.match(regexSearchWord) || user.status.match(regexSearchWord)) {
        result.push(user)
      }
  ...
  var currentZoom = parseFloat($('#svgChart').css('zoom'));

  /*$('.btn-zoomOut').on('click', function () {
    $('#svgChart').css('zoom', currentZoom -= 0.1);
    $('#svgChart').css('width', '100%');
  });
