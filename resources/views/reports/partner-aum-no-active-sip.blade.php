...
if(isset($data_table_headings_partner_aum_no_active_sip) && is_array($data_table_headings_partner_aum_no_active_sip) && count($data_table_headings_partner_aum_no_active_sip) > 0){
    foreach($data_table_headings_partner_aum_no_active_sip as $key => $value){
...
@endphp
@extends('../layout')
@section('title', 'Partners with AUM but no Active SIP Registrations')
@section('breadcrumb_heading', 'Partners with AUM but no Active SIP Registrations')
...
@section('custom_scripts')
    ...
    <script type="text/javascript">
        ...
        var partner_aum_no_active_sip_datatable;
        function load_project_focus_datatable(){
            ...
            partner_aum_no_active_sip_datatable = $('#panel_meetinglog').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX": true,
                "ajax": {
                    "url": baseurl +"/report-of-partner-aum-no-active-sip",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                        if(arn_number != null && typeof arn_number != 'undefined' && arn_number != ''){
            ...
            partner_aum_no_active_sip_datatable.columns().indexes().each(function(idx){
                $('#panel_meetinglog_wrapper table.dataTable thead tr:first th').eq(idx).find('input, select').on('change', function(){
                    ...
                    partner_aum_no_active_sip_datatable.column(idx).search(txtSearchedValue).draw();
                });
            });
        ...
        function export_csv_formatted_data(inputObj){
            ...
            formObj.attr({'action': baseurl + '/report-of-partner-aum-no-active-sip'});
            formObj.submit();
