...
if(isset($data_table_headings_project_focus) && is_array($data_table_headings_project_focus) && count($data_table_headings_project_focus) > 0){
    foreach($data_table_headings_project_focus as $key => $value){
...
@endphp
@extends('../layout')
@section('title', 'Project focus partners that we have not met in a quarter List')
@section('breadcrumb_heading', 'Project focus partners that we have not met in  a quarter List')
...
@section('custom_scripts')
    ...
    <script type="text/javascript">
        ...
        function load_project_focus_datatable(){
            ...
            project_focus_datatable = $('#panel_meetinglog').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX": true,
                "ajax": {
                    "url": baseurl +"/report-of-project-focus-partner",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                        if(arn_number != null && typeof arn_number != 'undefined' && arn_number != ''){
            ...
            project_focus_datatable.columns().indexes().each(function(idx){
                $('#panel_meetinglog_wrapper table.dataTable thead tr:first th').eq(idx).find('input, select').on('change', function(){
                    ...
                    project_focus_datatable.column(idx).search(txtSearchedValue).draw();
                });
            });
        ...
        function export_csv_formatted_data(inputObj){
            ...
            formObj.attr({'action': baseurl + '/report-of-project-focus-partner'});
            formObj.submit();
