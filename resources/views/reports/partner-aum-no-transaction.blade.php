...
if(isset($data_table_headings_partner_aum_no_transactions) && is_array($data_table_headings_partner_aum_no_transactions) && count($data_table_headings_partner_aum_no_transactions) > 0){
    foreach($data_table_headings_partner_aum_no_transactions as $key => $value){
...
@endphp
@extends('../layout')
@section('title', 'Partners with AUM but no recurring transactions')
@section('breadcrumb_heading', 'Partners with AUM but no recurring transactions')
...
@section('custom_scripts')
    ...
    <script type="text/javascript">
        ...
        var partner_aum_no_transaction_datatable;
        function load_project_focus_datatable(){
            ...
            $('#panel_meetinglog thead tr:first th').each( function (idx) {
                ...
                else if($.inArray(idx, [10]) != -1){
                    $(this).html( '<select><option value="">All</option><option value="0">No</option><option value="1">Yes</option></select>' );
                }
                else if($.inArray(idx, [6,7]) != -1){
                    $(this).html( '<select class="dropdown-select2"><option value="">All</option>@foreach($UserDetails as $key => $value)<option value="{{$value['id']}}">{{$value['name']}}</option>@endforeach</select>' );
                }
                else{
                    $(this).html('&nbsp;');
                }
            ...
            partner_aum_no_transaction_datatable = $('#panel_meetinglog').DataTable({
                "processing": true,
                "serverSide": true,
                "scrollX": true,
                "ajax": {
                    "url": baseurl +"/report-of-partner-aum-no-transactions",
                    "dataType": "json",
                    "type": "POST",
                    "data": function(d){
                        if(arn_number != null && typeof arn_number != 'undefined' && arn_number != ''){
                ...
                "columns": [
                            ...
                            {"data": "last_purchase_transaction_date",
                             "render": function (data, type, row, meta) {
                                    var inputColumn = 12, inputValue;
                                    var inputColumnName = meta['settings']['aoColumns'][inputColumn].data;
            ...
            partner_aum_no_transaction_datatable.columns().indexes().each(function(idx){
                $('#panel_meetinglog_wrapper table.dataTable thead tr:first th').eq(idx).find('input, select').on('change', function(){
                    ...
                    partner_aum_no_transaction_datatable.column(idx).search(txtSearchedValue).draw();
                });
            });
        ...
        function export_csv_formatted_data(inputObj){
            ...
            formObj.attr({'action': baseurl + '/report-of-partner-aum-no-transactions'});
            formObj.submit();
